  ...
  const onEnabledChange = value => {
    if (value || !hasContent) {
      ...
      Modal.confirm({
        ...
        content: __("Switching to default template will discard your custom template."),
        onOk: () => {
          setSubject(null);
          setBody(null);
          setEnabled(value);
      ...
      {!!enabled && (
        <div className="alert-custom-template" data-test="AlertCustomTemplate">
          <div className="d-flex align-items-center">
            <h5 className="flex-fill">{__("Subject / Body")}</h5>
            Preview{" "}
          ...
          <HelpTrigger type="ALERT_NOTIF_TEMPLATE_GUIDE" className="f-13">
            <i className="fa fa-question-circle" /> {__("Formatting guide")}
          </HelpTrigger>
        </div>
      )}
    </div>
